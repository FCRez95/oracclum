  ...
  try {
    ...
    if (!campaign_id || !days) {
      return NextResponse.json(
        { message: "Missing required query parameters: campaign_id, days" },
        { status: 400 }
      );
    }
    ...
    const backendUrl = `${ExternalURL}/load-ads-summary/${campaign_id}/${days}`;
    ...
    if (!backendResponse.ok) {
      ...
      return upstreamErrorResponse("Failed to fetch ads summary from backend", backendResponse.status);
    }
    ...
    return NextResponse.json(
      { message: "Failed to load campaign ads summary due to internal server error." },
      { status: 500 }
    );
  }
}
