...
import { deleteUser } from "@/app/(DataAccessLayer)/(appServices)/calls/admin/callDeleteUser";
...
export const DeleteUserModal = ({
  ...
  async function handleDelete() {
    setStatus("loading");
    try {
      const result = await CallerWrapper(deleteUser(userId));
      if (result.success) {
        setStatus("success");
        onDeleted(userId);
      } else {
      ...
      >
        ...
        <div className="flex flex-col h-full justify-between">
          <div>
            ...
            {status === "success" ? (
              <p>
                Usuário <strong>{userName}</strong> deletado com sucesso!
              </p>
            ) : hasError ? (
              <p>
                Erro ao deletar o usuário <strong>{userName}</strong>.
                Tente novamente.
              </p>
            ) : (
              <>
