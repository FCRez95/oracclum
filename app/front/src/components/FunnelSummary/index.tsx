...
const FunnelSummaryLine = ({
  ...
  const step1 = [
    { label: "Page views", value: stepsSummary.total_clicks },
    { label: "Abandonos", value: Math.max(step1Total - (stepsSummary.total_clicks ?? 0), 0) },
  ];

  const step2Total = stepsSummary.total_step_1 ?? 0;
  const step2 = [
    { label: "Page views", value: stepsSummary.step_1_views },
    { label: "Abandonos", value: Math.max(step2Total - (stepsSummary.step_1_views ?? 0), 0) },
  ];

  const step3Total = stepsSummary.total_step_2 ?? 0;
  const step3 = [
    { label: "Page views", value: stepsSummary.step_2_views },
    { label: "Abandonos", value: Math.max(step3Total - (stepsSummary.step_2_views ?? 0), 0) },
  ];

  const step4Total = stepsSummary.total_step_3 ?? 0;
  const step4 = [
    { label: "Page views", value: stepsSummary.step_3_views },
    { label: "Abandonos", value: Math.max(step4Total - (stepsSummary.step_3_views ?? 0), 0) },
  ];
  ...
  const steps = [
    ...
    ...(step4Total > 0 ? [{ title: "Passo 4", data: step4, total: step4Total }] : []),
