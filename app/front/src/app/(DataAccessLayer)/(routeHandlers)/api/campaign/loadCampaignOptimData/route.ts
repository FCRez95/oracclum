...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${ExternalURL}/load-campaign-optimization-data/${campaign_id}/${days}`;
    ...
    return NextResponse.json(
      {
        message:
          "Failed to load user campaign optimization data due to internal server error.",
      },
      { status: 500 }
    );
  }
