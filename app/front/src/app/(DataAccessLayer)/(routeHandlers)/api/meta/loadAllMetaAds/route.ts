...
export async function GET(request: NextRequest) {
    try {
        ...
        const backendUrl = `${ExternalURL}/load-all-meta-ads/${campaign_id}/${days}/`;
        ...
        if (!backendResponse.ok) {
            ...
            return upstreamErrorResponse('Failed to fetch meta ads data from backend', backendResponse.status);
        }
        ...
        return NextResponse.json(
            { message: 'Failed to load meta ads data due to internal server error.' },
            { status: 500 }
        );
    }
}
