...
export default (router: Router, pool: Pool) => {
  ...
  // Taboola routes
  router.get('/load-ads-summary/:id_campaign/:days', auth, userConsentsAuth, adptRoute(makeLoadAdsSummaryController(pool)))
  router.get('/load-ad-summary/:id_campaign/:id_ads_taboola/:days', auth, userConsentsAuth, adptRoute(makeLoadOneAdSummaryController(pool)))

  // Meta Ads routes
  router.get('/load-all-meta-adsets/:id_campaign/:days', auth, userConsentsAuth, adptRoute(makeLoadAllMetaAdsetsController(pool)))
  router.get('/load-all-meta-ads/:id_campaign/:days', auth, userConsentsAuth, adptRoute(makeLoadAllMetaAdsController(pool)))
  router.get('/load-one-meta-ad/:id_campaign/:id_ad_meta/:days', auth, userConsentsAuth, adptRoute(makeLoadOneMetaAdController(pool)))
}
