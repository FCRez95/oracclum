...
export async function GET(request: NextRequest) {
  try {
    ...
    const ad_id = request.nextUrl.searchParams.get("ad_id");
    const days = request.nextUrl.searchParams.get("days");

    if (!campaign_id || !ad_id || !days) {
      return NextResponse.json(
        { message: "Missing required query parameters: campaign_id, ad_id, days" },
        { status: 400 }
      );
    }
    ...
    const backendUrl = `${ExternalURL}/load-one-meta-ad/${campaign_id}/${ad_id}/${days}`;
    ...
    if (!backendResponse.ok) {
      ...
      return upstreamErrorResponse("Failed to fetch meta ad summary from backend", backendResponse.status);
    }
    ...
    return NextResponse.json(
      { message: "Failed to load meta ad summary due to internal server error." },
      { status: 500 }
    );
  }
}
